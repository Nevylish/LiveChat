    ...
    async function generateLink() {
        ...
        try {
            ...
            const response = await fetch(
                `${apiBase}/api/token/generate?username=${encodeURIComponent(username)}&guildId=${encodeURIComponent(guildId)}`,
            );

            if (!response.ok) {
                const data = await response.json().catch(() => null);
                setError(data?.error ?? 'Impossible de générer le lien, réessayez plus tard.');
                if (data?.id) {
                    setErrorId(data.id);
                }
                return;
            ...
            const overlayBase = isLocal ? 'http://localhost:4000/overlay' : 'https://livechat.nevylish.fr/overlay.html';
            const link = `${overlayBase}?username=${username}&guildId=${guildId}&token=${token}${disableSplash ? '&noSplash=true' : ''}`;
                                ...
                                <div className="config-card">
                                    ...
                                    <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center">
                                        <button
                                            ...
                                            className="w-full rounded-full bg-foreground px-7 py-3 text-sm font-semibold text-background transition-opacity duration-200 hover:opacity-85 disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto"
                                        >
                                            {isGenerating ? 'Génération...' : 'Générer mon lien'}
                                        </button>

                                        {errorId === 'bot_not_in_guild' && (
                                            <a
                                                href="https://discord.com/oauth2/authorize?client_id=1379921658109890610"
                                                target="_blank"
                                                rel="noopener noreferrer"
                                                className="animate-pulse-discord flex w-full items-center justify-center gap-2 rounded-full px-7 py-3 text-center text-sm font-semibold text-white transition-shadow duration-200 hover:opacity-90 sm:w-auto"
                                            >
