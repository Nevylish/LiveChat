...
export default class StopAndClearCommand extends Command {
    ...
    private async broadcastToEveryone(interaction: ChatInputCommandInteraction): Promise<void> {
        ...
        try {
            this.client.livechat.io.to(streamers.map((s) => s.socketId)).emit('clear');
            ...
            Logger.error('StopAndClearCommand', err.message);
